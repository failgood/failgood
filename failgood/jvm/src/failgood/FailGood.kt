...
object FailGood {
    ...
    internal fun createAutoTestSuite(randomTestClass: KClass<*> = findCaller()): Suite? {
        ...
        val lastRun: FileTime? =
            try {
                Files.readAttributes(timeStampPath, BasicFileAttributes::class.java)
                    .lastModifiedTime()
        ...
        return if (classes.isNotEmpty()) Suite(classes.map { ObjectContextProvider(it) }) else null
    }
    ...
    fun runTest() {
        val classes =
            listOf(javaClass.classLoader.loadClass((findCallerName().substringBefore("Kt"))).kotlin)
...
internal fun findCallerSTE(): StackTraceElement =
    Throwable().stackTrace.first { ste ->
        ste.fileName?.let {
            !(it.endsWith("FailGood.kt") || it.endsWith("SourceInfo.kt") || it.endsWith("Types.kt")|| it.endsWith("Describe.kt"))
        } ?: true
    }
