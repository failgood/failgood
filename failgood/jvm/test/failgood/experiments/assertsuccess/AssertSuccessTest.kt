...
private fun <E> List<E>.containsExactly(vararg matcher: (E) -> Boolean): CheckResult<Unit> {
    ...
    return if (errors.isEmpty())
        ...
        CheckResult.Failure(errors.joinToString())
}
    ...
    val context = describe("assertSuccess") {
        ...
        describe("asserting on lists") {
            it("could use a lambda matcher") {
                ...
                val songs = listOf(Song("the cure", "close to me"), Song("bruce springsteen", "born in the USA"))

                assert(songs.containsExactly({ it.artist == "the cure" }, { it.title == "born in the USA" }))
            }
        }
    }
}
