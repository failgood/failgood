    ...
    val context =
        describe(ContextExecutor::class) {
            describe("with a typical valid root context") {
                ...
                describe("executing all the tests") {
                    ...
                    it("returns deferred test results") {
                        ...
                        val failed =
                            assertNotNull(testResults.filter { it.isFailure }.singleOrNull())
                        val skipped =
            ...
            describe("supports lazy execution") {
                it(
                    "postpones test execution until the deferred is awaited when lazy is set to true"
                ) {
            ...
            it("handles failing root contexts") {
                val ctx =
                    RootContext("root context") { throw RuntimeException("root context failed") }
