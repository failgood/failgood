...
class ContextFinderTest {
    ...
    val context = describe(rootName) {
        ...
        it(testName) {
            ...
            assert(result.suite.contextProviders.singleOrNull()?.getContexts()?.singleOrNull()?.name == rootName)
            assert(
        ...
        describe("test filtering") {
            it("supports a classpathRootSelector with a package filter") {
                ...
                val contextNames =
                    suiteAndFilters.suite.contextProviders.flatMap { it.getContexts() }.map { it.name }
