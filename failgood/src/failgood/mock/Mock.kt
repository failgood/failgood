...
/**
 * Return calls to a mock to check with your favorite assertion lib (or the assertion lib you use)
 * in combination with the [call] helper. This is an alternative to [verify]
 *
...
interface MockConfigureDSL<Mock> {
    ...
    @Suppress("UNCHECKED_CAST") fun <T> any(): T = null as T
...
internal class MockHandler(private val kClass: KClass<*>) : InvocationHandler {
    ...
    class MockReplyRecorderImpl<Type>(
        ...
        override fun returns(parameter: Type) {
            ...
            if (parameter != null) mockHandler.defineResult(call.method) { parameter }
        }
