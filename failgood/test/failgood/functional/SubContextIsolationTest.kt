            ...
            describe("on a root context with default isolation (true)") {
                ...
                describe("turning off isolation for a subcontext") {
                    it("executes all the tests in that context in one go") {
                        ...
                        assert(
                            singleEvent.containsExactlyInAnyOrder(
                                listOf("childContext", "test1", "test2")))
                    }
                            ...
                            Suite(
                                    testCollection("root") {
                                        ...
                                        describe(
                                            ...
                                            isolation = false,
                                            given = {
                                                // make sure that the block is not called twice
                                                // before
                    ...
                    it("calls callbacks at the correct time") {
                        ...
                        assert(
                            noIsolationRun.endsWith(
                                "no-isolation-afterEach", "no-isolation-autoClose"))
                        assert(e[1] == listOf("child with isolation", "test3"))
                        assert(e[2] == listOf("child with isolation", "test4"))
                    }
                }
                describe("error handling") {
                    ...
                    it("does not allow to turn isolation on when it was already off") {
                        val result =
                            Suite(
                                    testCollection(
                                        "root context without isolation", isolation = false) {
                                            describe(
